...
class com_costbenefitprojectionInstallerScript
{
	...
	{
		// [4228] Get Application object
		$app = JFactory::getApplication();

		// [4230] Get The Database object
		$db = JFactory::getDbo();

		// [4239] Create a new query object.
		$query = $db->getQuery(true);
		// [4241] Select id from content type table
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Company alias is found
		$query->where( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.company') );
		$db->setQuery($query);
		// [4247] Execute query to see if alias is found
		$db->execute();
		$company_found = $db->getNumRows();
		// [4250] Now check if there were any rows
		if ($company_found)
		{
			// [4253] Since there are load the needed  company type ids
			$company_ids = $db->loadColumn();
			// [4257] Remove Company from the content type table
			$company_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.company') );
			// [4259] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__content_types'));
			$query->where($company_condition);
			$db->setQuery($query);
			// [4264] Execute the query to remove Company items
			$company_done = $db->execute();
			if ($company_done);
			{
				// [4268] If succesfully remove Company add queued success message.
			...
			// [4274] Remove Company items from the contentitem tag map table
			$company_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.company') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($company_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Company items
			$company_done = $db->execute();
			if ($company_done);
			{
				// [4285] If succesfully remove Company add queued success message.
			...
			// [4291] Remove Company items from the ucm content table
			$company_condition = array( $db->quoteName('core_type_alias') . ' = ' . $db->quote('com_costbenefitprojection.company') );
			// [4293] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__ucm_content'));
			$query->where($company_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Company items
			$company_done = $db->execute();
			if ($company_done);
			{
				// [4302] If succesfully remove Company add queued success message.
				$app->enqueueMessage(JText::_('The (com_costbenefitprojection.company) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4308] Make sure that all the Company items are cleared from DB
			foreach ($company_ids as $company_id)
			{
				// [4313] Remove Company items from the ucm base table
				$company_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $company_id);
				// [4315] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_base'));
				$query->where($company_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Company items
				$db->execute();

				// [4324] Remove Company items from the ucm history table
				$company_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $company_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($company_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Company items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Service_provider alias is found
		...
		{
			// [4253] Since there are load the needed  service_provider type ids
			$service_provider_ids = $db->loadColumn();
			// [4257] Remove Service_provider from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Service_provider items
			$service_provider_done = $db->execute();
			if ($service_provider_done);
			{
				// [4268] If succesfully remove Service_provider add queued success message.
			...
			// [4274] Remove Service_provider items from the contentitem tag map table
			$service_provider_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.service_provider') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($service_provider_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Service_provider items
			$service_provider_done = $db->execute();
			if ($service_provider_done);
			{
				// [4285] If succesfully remove Service_provider add queued success message.
			...
			// [4291] Remove Service_provider items from the ucm content table
			...
			$query->where($service_provider_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Service_provider items
			$service_provider_done = $db->execute();
			if ($service_provider_done);
			{
				// [4302] If succesfully remove Service_provider add queued success message.
			...
			// [4308] Make sure that all the Service_provider items are cleared from DB
			foreach ($service_provider_ids as $service_provider_id)
			{
				// [4313] Remove Service_provider items from the ucm base table
				...
				$query->where($service_provider_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Service_provider items
				$db->execute();

				// [4324] Remove Service_provider items from the ucm history table
				$service_provider_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $service_provider_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($service_provider_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Service_provider items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Country alias is found
		...
		if ($country_found)
		{
			// [4253] Since there are load the needed  country type ids
			$country_ids = $db->loadColumn();
			// [4257] Remove Country from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Country items
			$country_done = $db->execute();
			if ($country_done);
			{
				// [4268] If succesfully remove Country add queued success message.
			...
			// [4274] Remove Country items from the contentitem tag map table
			$country_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.country') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($country_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Country items
			$country_done = $db->execute();
			if ($country_done);
			{
				// [4285] If succesfully remove Country add queued success message.
			...
			// [4291] Remove Country items from the ucm content table
			...
			$query->where($country_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Country items
			$country_done = $db->execute();
			if ($country_done);
			{
				// [4302] If succesfully remove Country add queued success message.
				$app->enqueueMessage(JText::_('The (com_costbenefitprojection.country) type alias was removed from the <b>#__ucm_content</b> table'));
			}

			// [4308] Make sure that all the Country items are cleared from DB
			foreach ($country_ids as $country_id)
			{
				// [4313] Remove Country items from the ucm base table
				...
				$query->where($country_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Country items
				$db->execute();

				// [4324] Remove Country items from the ucm history table
				$country_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $country_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($country_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Country items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Causerisk alias is found
		...
		if ($causerisk_found)
		{
			// [4253] Since there are load the needed  causerisk type ids
			$causerisk_ids = $db->loadColumn();
			// [4257] Remove Causerisk from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Causerisk items
			$causerisk_done = $db->execute();
			if ($causerisk_done);
			{
				// [4268] If succesfully remove Causerisk add queued success message.
			...
			// [4274] Remove Causerisk items from the contentitem tag map table
			$causerisk_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.causerisk') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($causerisk_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Causerisk items
			$causerisk_done = $db->execute();
			if ($causerisk_done);
			{
				// [4285] If succesfully remove Causerisk add queued success message.
			...
			// [4291] Remove Causerisk items from the ucm content table
			...
			$query->where($causerisk_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Causerisk items
			$causerisk_done = $db->execute();
			if ($causerisk_done);
			{
				// [4302] If succesfully remove Causerisk add queued success message.
			...
			// [4308] Make sure that all the Causerisk items are cleared from DB
			foreach ($causerisk_ids as $causerisk_id)
			{
				// [4313] Remove Causerisk items from the ucm base table
				...
				$query->where($causerisk_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Causerisk items
				$db->execute();

				// [4324] Remove Causerisk items from the ucm history table
				$causerisk_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $causerisk_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($causerisk_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Causerisk items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Health_data alias is found
		...
		{
			// [4253] Since there are load the needed  health_data type ids
			$health_data_ids = $db->loadColumn();
			// [4257] Remove Health_data from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Health_data items
			$health_data_done = $db->execute();
			if ($health_data_done);
			{
				// [4268] If succesfully remove Health_data add queued success message.
			...
			// [4274] Remove Health_data items from the contentitem tag map table
			$health_data_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.health_data') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($health_data_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Health_data items
			$health_data_done = $db->execute();
			if ($health_data_done);
			{
				// [4285] If succesfully remove Health_data add queued success message.
			...
			// [4291] Remove Health_data items from the ucm content table
			...
			$query->where($health_data_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Health_data items
			$health_data_done = $db->execute();
			if ($health_data_done);
			{
				// [4302] If succesfully remove Health_data add queued success message.
			...
			// [4308] Make sure that all the Health_data items are cleared from DB
			foreach ($health_data_ids as $health_data_id)
			{
				// [4313] Remove Health_data items from the ucm base table
				...
				$query->where($health_data_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Health_data items
				$db->execute();

				// [4324] Remove Health_data items from the ucm history table
				$health_data_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $health_data_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($health_data_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Health_data items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Scaling_factor alias is found
		...
		{
			// [4253] Since there are load the needed  scaling_factor type ids
			$scaling_factor_ids = $db->loadColumn();
			// [4257] Remove Scaling_factor from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Scaling_factor items
			$scaling_factor_done = $db->execute();
			if ($scaling_factor_done);
			{
				// [4268] If succesfully remove Scaling_factor add queued success message.
			...
			// [4274] Remove Scaling_factor items from the contentitem tag map table
			$scaling_factor_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.scaling_factor') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($scaling_factor_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Scaling_factor items
			$scaling_factor_done = $db->execute();
			if ($scaling_factor_done);
			{
				// [4285] If succesfully remove Scaling_factor add queued success message.
			...
			// [4291] Remove Scaling_factor items from the ucm content table
			...
			$query->where($scaling_factor_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Scaling_factor items
			$scaling_factor_done = $db->execute();
			if ($scaling_factor_done);
			{
				// [4302] If succesfully remove Scaling_factor add queued success message.
			...
			// [4308] Make sure that all the Scaling_factor items are cleared from DB
			foreach ($scaling_factor_ids as $scaling_factor_id)
			{
				// [4313] Remove Scaling_factor items from the ucm base table
				...
				$query->where($scaling_factor_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Scaling_factor items
				$db->execute();

				// [4324] Remove Scaling_factor items from the ucm history table
				$scaling_factor_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $scaling_factor_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($scaling_factor_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Scaling_factor items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Intervention alias is found
		...
		{
			// [4253] Since there are load the needed  intervention type ids
			$intervention_ids = $db->loadColumn();
			// [4257] Remove Intervention from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Intervention items
			$intervention_done = $db->execute();
			if ($intervention_done);
			{
				// [4268] If succesfully remove Intervention add queued success message.
			...
			// [4274] Remove Intervention items from the contentitem tag map table
			$intervention_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.intervention') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($intervention_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Intervention items
			$intervention_done = $db->execute();
			if ($intervention_done);
			{
				// [4285] If succesfully remove Intervention add queued success message.
			...
			// [4291] Remove Intervention items from the ucm content table
			...
			$query->where($intervention_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Intervention items
			$intervention_done = $db->execute();
			if ($intervention_done);
			{
				// [4302] If succesfully remove Intervention add queued success message.
			...
			// [4308] Make sure that all the Intervention items are cleared from DB
			foreach ($intervention_ids as $intervention_id)
			{
				// [4313] Remove Intervention items from the ucm base table
				...
				$query->where($intervention_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Intervention items
				$db->execute();

				// [4324] Remove Intervention items from the ucm history table
				$intervention_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $intervention_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($intervention_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Intervention items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Currency alias is found
		...
		if ($currency_found)
		{
			// [4253] Since there are load the needed  currency type ids
			$currency_ids = $db->loadColumn();
			// [4257] Remove Currency from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Currency items
			$currency_done = $db->execute();
			if ($currency_done);
			{
				// [4268] If succesfully remove Currency add queued success message.
			...
			// [4274] Remove Currency items from the contentitem tag map table
			$currency_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.currency') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($currency_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Currency items
			$currency_done = $db->execute();
			if ($currency_done);
			{
				// [4285] If succesfully remove Currency add queued success message.
			...
			// [4291] Remove Currency items from the ucm content table
			...
			$query->where($currency_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Currency items
			$currency_done = $db->execute();
			if ($currency_done);
			{
				// [4302] If succesfully remove Currency add queued success message.
			...
			// [4308] Make sure that all the Currency items are cleared from DB
			foreach ($currency_ids as $currency_id)
			{
				// [4313] Remove Currency items from the ucm base table
				...
				$query->where($currency_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Currency items
				$db->execute();

				// [4324] Remove Currency items from the ucm history table
				$currency_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $currency_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($currency_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Currency items
				$db->execute();
			}
		}
		...
		$query->select($db->quoteName('type_id'));
		$query->from($db->quoteName('#__content_types'));
		// [4244] Where Help_document alias is found
		...
		{
			// [4253] Since there are load the needed  help_document type ids
			$help_document_ids = $db->loadColumn();
			// [4257] Remove Help_document from the content type table
			...
			$db->setQuery($query);
			// [4264] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [4268] If succesfully remove Help_document add queued success message.
			...
			// [4274] Remove Help_document items from the contentitem tag map table
			$help_document_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_costbenefitprojection.help_document') );
			// [4276] Create a new query object.
			$query = $db->getQuery(true);
			$query->delete($db->quoteName('#__contentitem_tag_map'));
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [4281] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [4285] If succesfully remove Help_document add queued success message.
			...
			// [4291] Remove Help_document items from the ucm content table
			...
			$query->where($help_document_condition);
			$db->setQuery($query);
			// [4298] Execute the query to remove Help_document items
			$help_document_done = $db->execute();
			if ($help_document_done);
			{
				// [4302] If succesfully remove Help_document add queued success message.
			...
			// [4308] Make sure that all the Help_document items are cleared from DB
			foreach ($help_document_ids as $help_document_id)
			{
				// [4313] Remove Help_document items from the ucm base table
				...
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [4320] Execute the query to remove Help_document items
				$db->execute();

				// [4324] Remove Help_document items from the ucm history table
				$help_document_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $help_document_id);
				// [4326] Create a new query object.
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__ucm_history'));
				$query->where($help_document_condition);
				$db->setQuery($query);
				// [4331] Execute the query to remove Help_document items
				$db->execute();
			}
		}

		// [4339] If All related items was removed queued success message.
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_base</b> table'));
		$app->enqueueMessage(JText::_('All related items was removed from the <b>#__ucm_history</b> table'));

		// [4344] Remove costbenefitprojection assets from the assets table
		$costbenefitprojection_condition = array( $db->quoteName('name') . ' LIKE ' . $db->quote('com_costbenefitprojection%') );

		// [4346] Create a new query object.
		$query = $db->getQuery(true);
		...
		if ($help_document_done);
		{
			// [4354] If succesfully remove costbenefitprojection add queued success message.
	...
	function postflight($type, $parent)
	{
		...
		if ($type == 'install')
		{
			...
			// [4400] Get The Database object

			$db = JFactory::getDbo();

			// [4407] Create the company content type object.
			...
			// [4413] Insert the object into the content types table.
			$companyInserted = $db->insertObject('#__content_types', $company);

			// [4407] Create the service_provider content type object.
			...
			// [4407] Create the country content type object.
			...
			// [4407] Create the causerisk content type object.
			...
			// [4407] Create the health_data content type object.
			...
			// [4407] Create the scaling_factor content type object.
			...
			// [4407] Create the intervention content type object.
			...
			// [4407] Create the currency content type object.
			...
			// [4407] Create the help_document content type object.
			...
			// [4425] Install the global extenstion params.
			$query = $db->getQuery(true);

			// [4433] Field to update.
			$fields = array(
		...
		if ($type == 'update')
		{
			echo '<a target="_blank" href="http://www.vdm.io" title="Cost Benefit Projection">
				...
				<h3>Upgrade to Version (3.0.9) Was Successful!</h3>';
		}
	}
}
