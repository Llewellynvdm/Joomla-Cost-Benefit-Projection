...
{    
	...
	public function getItem($pk = null)
	{
		...
		$this->serviceproviderrnkc = $item->id;
	...
	*/
	public function getZducompanies()
	{
		...
		// [6982] Filter by serviceproviderrnkc global.
		$serviceproviderrnkc = $this->serviceproviderrnkc;
		if (is_numeric($serviceproviderrnkc ))
		{
			$query->where('a.serviceprovider = ' . (int) $serviceproviderrnkc );
		}
		elseif (is_string($serviceproviderrnkc))
		{
			$query->where('a.serviceprovider = ' . $db->quote($serviceproviderrnkc));
		}
		else
		{
		...
		if ($db->getNumRows())
		{
			...
			if (CostbenefitprojectionHelper::checkArray($items))
			{
				foreach ($items as $nr => &$item)
				{
					...
					$item->department = $this->selectionTranslationZducompanies($item->department, 'department');
					// [10893] convert per
					$item->per = $this->selectionTranslationZducompanies($item->per, 'per');
				}
			}
	...
	*/
	public function selectionTranslationZducompanies($value,$name)
	{
