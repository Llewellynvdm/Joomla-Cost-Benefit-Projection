...
{    
	...
	public function getItem($pk = null)
	{
		...
		$this->serviceprovidernhre = $item->id;
	...
	*/
	public function getSvccompanies()
	{
		// Get the user object.
		...
		// Filter by serviceprovidernhre global.
		$serviceprovidernhre = $this->serviceprovidernhre;
		if (is_numeric($serviceprovidernhre ))
		{
			$query->where('a.serviceprovider = ' . (int) $serviceprovidernhre );
		}
		elseif (is_string($serviceprovidernhre))
		{
			$query->where('a.serviceprovider = ' . $db->quote($serviceprovidernhre));
		}
		else
		{
		...
		if ($db->getNumRows())
		{
			...
			if (CostbenefitprojectionHelper::checkArray($items))
			{
				...
				{
					// convert department
					$item->department = $this->selectionTranslationSvccompanies($item->department, 'department');
					// convert per
					$item->per = $this->selectionTranslationSvccompanies($item->per, 'per');
				}
			}
	...
	*/
	public function selectionTranslationSvccompanies($value,$name)
	{
