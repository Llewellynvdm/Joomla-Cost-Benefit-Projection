...
{    
	...
	public function getItem($pk = null)
	{
		...
		$this->serviceprovidermdas = $item->id;
	...
	*/
	public function getQwycompanies()
	{
		...
		// [7234] Filter by serviceprovidermdas global.
		$serviceprovidermdas = $this->serviceprovidermdas;
		if (is_numeric($serviceprovidermdas ))
		{
			$query->where('a.serviceprovider = ' . (int) $serviceprovidermdas );
		}
		elseif (is_string($serviceprovidermdas))
		{
			$query->where('a.serviceprovider = ' . $db->quote($serviceprovidermdas));
		}
		else
		{
		...
		if ($db->getNumRows())
		{
			...
			if (CostbenefitprojectionHelper::checkArray($items))
			{
				foreach ($items as $nr => &$item)
				{
					...
					$item->department = $this->selectionTranslationQwycompanies($item->department, 'department');
					// [11149] convert per
					$item->per = $this->selectionTranslationQwycompanies($item->per, 'per');
				}
			}
	...
	*/
	public function selectionTranslationQwycompanies($value,$name)
	{
