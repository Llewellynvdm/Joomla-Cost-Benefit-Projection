...
{    
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_costbenefitprojection.health_data';
	...
	{		// [9651] Get the form.
		...
		// [9810] Modify the form based on Edit Causerisk access controls.
		...
		// [9810] Modify the form based on Edit Year access controls.
		...
		// [9810] Modify the form based on Edit Femaledeath access controls.
		...
		// [9810] Modify the form based on Edit Maleyld access controls.
		...
		// [9810] Modify the form based on Edit Maledeath access controls.
		...
		// [9810] Modify the form based on Edit Femaleyld access controls.
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__costbenefitprojection_health_data'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_costbenefitprojection.edit.health_data.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('health_data.create') && !$this->canDo->get('health_data.batch'))
		{
			return false;
		}
		...
		{
			// make absolutely sure that this health data can be copied
			$is = CostbenefitprojectionHelper::userIs($this->user->id);
			...
			if ((3 == $is) && CostbenefitprojectionHelper::checkArray($countries))
			{
				...
				{
					$country = CostbenefitprojectionHelper::getId('health_data',$pk,'id','country');
		...
		elseif (isset($values['published']) && !$this->canDo->get('health_data.edit.state'))
		{
				$values['published'] = 0;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('health_data.edit') && !$this->canDo->get('health_data.batch'))
		{
			$this->setError(JText::_('JLIB_APPLICATION_ERROR_BATCH_CANNOT_EDIT'));
			return false;
		}
		...
		if (!$this->user->authorise('core.options', 'com_costbenefitprojection'))
		{
			// make absolutely sure that this health data can be moved
			$is = CostbenefitprojectionHelper::userIs($this->user->id);
			$countries = CostbenefitprojectionHelper::hisCountries($this->user->id);
			if ((3 == $is) && CostbenefitprojectionHelper::checkArray($countries))
			{
