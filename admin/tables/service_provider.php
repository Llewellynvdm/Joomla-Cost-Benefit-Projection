...
/**
 * Service_providers Table class
 */
class CostbenefitprojectionTableService_provider extends JTable
{
	/**
	...
	/**
	 * Overload the store method for the Service_provider table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New service_provider. A service_provider created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('service_provider', 'CostbenefitprojectionTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_COSTBENEFITPROJECTION_SERVICE_PROVIDER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_costbenefitprojection.service_provider.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_costbenefitprojection.service_provider.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_costbenefitprojection');
