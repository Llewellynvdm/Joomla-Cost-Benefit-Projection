 ...
 */
class CostbenefitprojectionTableHealth_data extends JTable
{
	/**
	...
	/**
	 * Overload the store method for the Health_data table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New health_data. A health_data created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('health_data', 'CostbenefitprojectionTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_COSTBENEFITPROJECTION_HEALTH_DATA_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_costbenefitprojection.health_data.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_costbenefitprojection.health_data.'.(int) $this->$k;
	}
