...
/**
 * Scaling_factors Table class
 */
class CostbenefitprojectionTableScaling_factor extends JTable
{
	/**
	...
	/**
	 * Overload the store method for the Scaling_factor table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New scaling_factor. A scaling_factor created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('scaling_factor', 'CostbenefitprojectionTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_COSTBENEFITPROJECTION_SCALING_FACTOR_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_costbenefitprojection.scaling_factor.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_costbenefitprojection.scaling_factor.'.(int) $this->$k;
	}
