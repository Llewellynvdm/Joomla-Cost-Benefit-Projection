...
class CostbenefitprojectionViewHelp_documents extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		if ($this->admin_viewOptions)
		{
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [9239] Admin View Batch Selection
				JHtmlBatch_::addListSelection(
		...
		if ($this->site_viewOptions)
		{
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [9239] Site View Batch Selection
				JHtmlBatch_::addListSelection(
	...
	protected function setDocument()
	{
		...
		$document->setTitle(JText::_('COM_COSTBENEFITPROJECTION_HELP_DOCUMENTS'));
		$document->addStyleSheet(JURI::root() . "administrator/components/com_costbenefitprojection/assets/css/help_documents.css");
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.admin_view' => JText::_('COM_COSTBENEFITPROJECTION_HELP_DOCUMENT_ADMIN_VIEW_LABEL'),
			'a.site_view' => JText::_('COM_COSTBENEFITPROJECTION_HELP_DOCUMENT_SITE_VIEW_LABEL'),
	...
	protected function getTheTypeSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [9129] Translate the type selection
				$text = $model->selectionTranslation($type,'type');
				// [9131] Now add the type and its text to the options array
	...
	protected function getTheLocationSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [9129] Translate the location selection
				$text = $model->selectionTranslation($location,'location');
				// [9131] Now add the location and its text to the options array
	...
	protected function getTheAdmin_viewSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [9136] Now add the admin_view and its text to the options array
	...
	protected function getTheSite_viewSelections()
	{
		...
		if ($results)
		{
			...
			{
				// [9136] Now add the site_view and its text to the options array
